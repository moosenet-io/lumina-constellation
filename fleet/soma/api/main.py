...
try:
    ...
    import security as security_api
...
def _auth(x_soma_key: str = "", request: Request = None):
    ...
    if request is not None:
        ...
        if cookie and _AUTH_AVAILABLE and _auth_module is not None:
            try:
                user = _auth_module.verify_token(cookie)
                if user:
...
async def get_status():
    """Comprehensive system status — served from cache (10s TTL) with background refresh.
    Delegates to refresh_status.py when available (services format), falls back to legacy checks."""
    import time
    ...
    if _CACHE_AVAILABLE:
        ...
        if cached is not None:
            return cached
    # Use the new refresher if available — it returns 'services' format with agents, refractor, synapse
    if _refresh_status_fn is not None:
        try:
            result = _refresh_status_fn()
            if _CACHE_AVAILABLE:
