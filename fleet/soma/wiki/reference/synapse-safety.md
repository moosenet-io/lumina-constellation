...
Synapse messages appear in Peter's personal Matrix channel — the same place he manages his day. A bad Synapse message at the wrong moment is more disruptive than silence. These rules exist to keep Synapse trustworthy and non-intrusive.
...
Synapse does not comment on sleep patterns, mood, stress indicators, or anything health-related unless Peter explicitly asks Lumina about these topics in conversation. Proactive health "nudges" feel surveillance-like even when well-intentioned.

### 2. Relationship or personal life
Calendar events involving people (dinners, calls, family) are informational context only. Synapse never surfaces observations about Peter's personal relationships.

### 3. Criticism or negative evaluations
Synapse never proactively criticizes decisions Peter has already made. Retrospective critique ("you should have done X") is not in scope — that's what asking Lumina is for.
...
If Peter dismissed the last message from a given trigger without acting on it, that trigger enters a 48-hour cooldown. Synapse does not retry dismissed messages.
...
**Wrong**: "Hi Peter! I noticed that the Vector task you submitted yesterday has been idle for more than 6 hours. I wanted to flag this in case you were wondering what was happening with it and whether you'd like me to check on it or possibly abort it."
...
Messages should either (a) suggest a clear action or (b) be information Peter can acknowledge and move on. Messages that require Peter to go research something to understand them are noise.

### Do not repeat context Peter already has
...
When Peter responds to a Synapse message, Lumina classifies the response:

| Peter's Response | Classification | Synapse Action |
|------------------|---------------|----------------|
...
4. Enable with a 24-hour probation: Peter explicitly reviews Synapse messages for the first day
5. Adjust sensitivity / cooldown based on Peter's feedback
...
Peter can disable Synapse globally or per-trigger:
...
Global disable should be the nuclear option. If Peter needs to disable Synapse, something has gone wrong with trigger calibration and that should be fixed, not papered over.
