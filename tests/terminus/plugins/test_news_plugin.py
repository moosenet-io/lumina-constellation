"""Tests for terminus/plugins/news_plugin.py"""
import pytest
from pathlib import Path

# Import module via path to verify it is importable (scaffold only).
from tests.conftest import import_module_from_path
MODULE_PATH = Path(r'/opt/lumina/lumina-constellation/terminus/plugins/news_plugin.py')
try:
    module = import_module_from_path('terminus_plugins_news_plugin', str(MODULE_PATH))
    _import_error = None
